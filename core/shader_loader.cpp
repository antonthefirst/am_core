...
static void checkForUpdates(int file_idx, StringRange path) {
	...
	// TEXT
	// 0
	size_t prepend_len = 10 + file_entries[file_idx].name.len + 2;
	file_entries[file_idx].text = (char*)malloc(prepend_len + text_len + 1);
	int w = sprintf(file_entries[file_idx].text, "\n#line 1 \"%s\"\n", file_entries[file_idx].name.str);
	assert(w == (int) prepend_len);
	if (file_entries[file_idx].is_procedural)
		memcpy(file_entries[file_idx].text + prepend_len, file_entries[file_idx].procedural_text, text_len);
	else
		fread(file_entries[file_idx].text + prepend_len, text_len, 1, f);
	file_entries[file_idx].text[prepend_len + text_len] = 0;
	file_entries[file_idx].text_len = prepend_len + text_len;
	...
	// state 4 = read string
	// reading string
	const char* t = file_entries[file_idx].text + prepend_len;
	int ctr = 0;
	char buff[1024];
	int file_start = 0;
	if (path) {
...
static void checkAllFilesForUpdates() {
	...
	for (ShaderEntry& s : shader_entries) {
		if (s.stale) {
			...
			/*
			FILE* F = fopen("src/sdf_render/concat/render.comp", "w");
			for (int i = 0; i < texts.size(); ++i) {
				fwrite(texts[i], lens[i], 1, F);
			}
			fclose(F);
			*/
	...
	for (int i = 0; i < (int)prog_entries.size(); ++i) {
		...
		if (p.try_relink) {
			...
			if (params == GL_TRUE) {
				...
				int log_len = 0;
				glGetShaderInfoLog(new_handle, p.log.max_bytes, &log_len, p.log.str);
				p.log.len = log_len;
